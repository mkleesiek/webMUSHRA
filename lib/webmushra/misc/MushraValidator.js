...
/**
 * Duration of stimulus must not exceed 30s.
 */
MushraValidator.prototype.checkStimulusDuration = function(_stimulus) {
  if (_stimulus.getAudioBuffer().duration > 30.0) {
    this.errorHandler.sendError("Duration of stimulus (" + _stimulus.getId() + ") must not exceed 30s.");
    return false;
  }
  return true;
};
